...
class Flatten3DGuidance(BaseObject):
                ...
                with torch.no_grad():
                    ...
                    for i, b in enumerate(range(0, len(latents), camera_batch_size)):
                        ...
                        for down_sample_factor in [1, 2, 4, 8]:
                            ...
                            depth_correspondence[H * W] = []
                            depth_valid_mask[H * W] = []
                            for cam_index, cam in enumerate(cams[b:b + camera_batch_size], b):
                                ...
                                depth_valid_mask[H * W].append(torch.stack(cam_depth_valid_mask, dim=0))
                            depth_correspondence[H * W] = torch.stack(depth_correspondence[H * W], dim=0)
                            depth_valid_mask[H * W] = torch.stack(depth_valid_mask[H * W], dim=0)
                        register_depth_correspondence(self.unet, depth_correspondence, depth_valid_mask)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        depth_latents = depth_map.permute(0, 3, 1, 2).to(torch.float16).cuda()
